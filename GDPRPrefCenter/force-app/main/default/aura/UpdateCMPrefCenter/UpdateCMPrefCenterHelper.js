({
    configMap: {
        "string": { 
            componentDef: "lightning:inputField",
            attributes: { 
    ...
    onInit : function(component, event, helper){
        ...
        var schools_Colleges = "schools_Colleges";
        var schools_Colleges_Tepper = "schools_Colleges_Tepper";
        
        var categoryFieldset = self.createFieldset(component, event, helper, category);
        var schools_CollegesFieldset = self.createFieldset(component, event, helper, schools_Colleges);
        var schools_Colleges_TepperFieldset = self.createFieldset(component, event, helper, schools_Colleges_Tepper);
        
        var identity_NetworksFielset = self.createFieldset(component, event, helper, "Identity_Networks");
        //var other_NetworkFielset = self.createFieldset(component, event, helper, "Other_Network");
        var student_OrganizationsFielset = self.createFieldset(component, event, helper, "Student_Organizations");
        
    },
    
    createFieldset : function(component, event, helper, fieldSetName){
        var self = this;
        var form = [];
        ...
        FiledSetMember.setCallback(this, function(response){
            ...
            if(component.isValid() && (state === 'SUCCESS' || state === 'DRAFT')){
                ...
                self.createForm(component, event, helper, fieldSetMember, fieldSetName);
    ...
    createForm : function(component, event, helper, fieldSetMember, fieldSetName) {
        var inputDesc = [];
        var config = null;
        
        for(var i=0; i < fieldSetMember.length; i++) {
            if(fieldSetMember[i].fieldType.toLowerCase() === 'string'){
                config = JSON.parse(
                    JSON.stringify(this.configMap['string'])
                );
                config.attributes.fieldName = fieldSetMember[i].fieldAPIName;
                inputDesc.push([
                    config.componentDef,
                    config.attributes
                ]);
                ...
                config.attributes.fieldName = fieldSetMember[i].fieldAPIName;
                config.attributes["aura:id"] = fieldSetName + "checkbox";
                config.attributes["onchange"] = component.getReference("c.onFieldMemberChange");
                inputDesc.push([
                    config.componentDef,
                    config.attributes
                ]);
                    ...
                    config.attributes.fieldName = fieldSetMember[i].fieldAPIName;
                    config.attributes["aura:id"] = fieldSetName + fieldSetMember[i].fieldAPIName;
                    if(fieldSetMember[i].fieldAPIName.toLowerCase() === 'preferred_region_1__c') {
        ...
        $A.createComponents(inputDesc,
                            function(components, status, errorMessage){
                                if (status === "SUCCESS") {                                    
                                    ...
                                    component.set("v."+fieldSetName, form);
    ...
    changeFieldSetMemberValues : function(component, event, helper, fieldsetName , value) { 
        var categoryFieldset = this.updateFieldset(component, event, helper, fieldsetName);          
    },
    
    updateForm : function(component, event, helper, fieldSetMember, fieldSetName) {
        var inputDesc = [];
        var config = null;
    ...
    updateFieldset : function(component, event, helper, fieldSetName){
        var self = this;
        var form = [];
        ...
        FiledSetMember.setCallback(this, function(response){
            ...
            if(component.isValid() && (state === 'SUCCESS' || state === 'DRAFT')){
                var fieldSetMember = JSON.parse(response.getReturnValue());              
                self.updateForm(component, event, helper, fieldSetMember, fieldSetName);
    ...
    getMaskedEmail : function(component, event, helper) {
        try {
            var email=component.get("v.email");
            var arr=email.split("@");
            var newEmail=arr[0].slice(0,3);
            for (var i = 0; i < arr[0].slice(3).length; i++) {
                newEmail=newEmail+"*"; 
            }
            newEmail=newEmail+"@"; 
            ...
            component.set("v.emailMasked",newEmail);
        }catch(err) {
        }
        
    },
        getchangeFieldSetMemberValues : function(component, event, helper) {
        try {
                /*this.changeFieldSetMemberValues(component, event, helper,"category", false);
                this.changeFieldSetMemberValues(component, event, helper,"schools_Colleges", false);
                this.changeFieldSetMemberValues(component, event, helper,"schools_Colleges_Tepper", false);
                this.changeFieldSetMemberValues(component, event, helper,"Identity_Networks", false);
                this.changeFieldSetMemberValues(component, event, helper,"Other_Network", false);
                this.changeFieldSetMemberValues(component, event, helper,"Student_Organizations", false);*/
        }catch(err) {
        }
    ...
    getRegionPicklist : function(component, event, helper){
        var FiledSetMember = component.get('c.getRegionValues');
        
        FiledSetMember.setCallback(this, function(response){
            var state = response.getState();
            if(component.isValid() && (state === 'SUCCESS' || state === 'DRAFT')){
                var reg = response.getReturnValue();              
                component.set('v.regionPicklist', reg);
 ...
 getCountryOptions: function() {
    ...
    createMap : function(cmp, event, helper) {
        ...
        var myArray = original["countries"];
        var CoolMap = {};
        
        var CountriesArray = []
        for (var i in myArray){ 
            var CountriesMap = {};
            CountriesMap["label"] = myArray[i].country;
            CountriesMap["value"] = myArray[i].country;
			CountriesArray.push(CountriesMap)
            
            var myStates=[];
            for (var j in myArray[i].states){
                
                var myMap ={};
                myMap["label"]= myArray[i].states[j];
                myMap["value"] = myArray[i].states[j];
                myStates.push(myMap);
            }
            CoolMap[myArray[i].country]= myStates;
            this.countryProvinceMap = CoolMap;
            this.countryOptions = CountriesArray;
